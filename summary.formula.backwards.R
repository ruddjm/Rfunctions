...
# This is a function to do summary.formula, with method = "reverse", but with the percentages calculated for the rows 
# rather than by columns.

## overall must be set to FALSE in the original call to summary.formula that is used as an argument to these functions

formatCatsBackward <- function(tab, nam, tr, type, group.freq, npct, pctdig, exclude1, 
    long, prtest, latex = FALSE, testUsed = character(0), npct.size = "scriptsize", 
    pdig = 3, eps = 0.001, footnoteTest = TRUE, dotchart = FALSE) 

#            stats[[i]], nam, tr, type[i], if (length(x$group.freq)) 
         #       x$group.freq
          #  else x$n[i], npct, pctdig, exclude1, long, prtest, 
           #     latex = TRUE, testUsed = testUsed, npct.size = npct.size, 
...
latex.summary.formula.reverse.backwards <- function(object, title = first.word(deparse(substitute(object))), 
    digits, prn = any(n != N), pctdig = 0, npct = c("numerator", 
        "both", "denominator", "none"), npct.size = "scriptsize", 
    Nsize = "scriptsize", exclude1 = TRUE, vnames = c("labels", 
        "names"), prUnits = TRUE, middle.bold = FALSE, outer.size = "scriptsize", 
    caption, rowlabel = "", insert.bottom = TRUE, dcolumn = FALSE, 
    formatArgs = NULL, round = NULL, prtest = c("P", "stat", 
        "df", "name"), prmsd = FALSE, msdsize = NULL, long = dotchart, 
    pdig = 3, eps = 0.001, auxCol = NULL, dotchart = FALSE, ...) 
{
    x <- object
    npct <- match.arg(npct)
    ...
    for (i in 1:nv) {
        ...
        if (type[i] == 1 || type[i] == 3) {
            cs <- formatCatsBackward(stats[[i]], nam, tr, type[i], if (length(x$group.freq)) 
                x$group.freq
            else x$n[i], npct, pctdig, exclude1, long, prtest, 
                latex = TRUE, testUsed = testUsed, npct.size = npct.size, 
                pdig = pdig, eps = eps, footnoteTest = gt1.test, 
    ...
    if (length(test) && !all(prtest == "none")) {
        gl <- c(gl, if (length(prtest) == 1 && prtest != "stat") if (prtest == 
            "P") "P-value" else prtest else "Test Statistic")
    ...
    if (!insert.bottom) 
        ...
        if (any(type == 2)) {
            legend <- paste("\\noindent {\\", outer.size, " $a$\\ }{", 
                bld, "$b$\\ }{\\", outer.size, " $c$\\ } represent the lower quartile $a$, the median $b$, and the upper quartile $c$\\ for continuous variables.", 
                if (prmsd) 
    ...
    resp <- latex.default(cstats, title = title, caption = caption, 
        rowlabel = rowlabel, col.just = col.just, numeric.dollar = FALSE, 
